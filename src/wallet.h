...
public:
    ...
    int64_t GetBitcoinBalance() const;
    ...
    bool SendBtcToAnon(CStealthAddress& sxAddress, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee=false);
    ...
    bool SendAnonToBtc(CStealthAddress& sxAddress, int64_t nValue, int nRingSize, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee=false);
    ...
    int64_t GetBitcoinDebit(const CTxIn& txin) const;
    int64_t GetBitcoinCredit(const CTxOut& txout) const;
    ...
    int64_t GetCredit(const CTransaction& tx) const
    {
        ...
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nCredit += GetBitcoinCredit(txout);
            } else
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nBTC, int64_t& nBitcoin) const
    {
        nBTC = 0;
        nBitcoin = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nBitcoin += GetBitcoinCredit(txout);
            } else
                nBTC += GetCredit(txout);
            if (!MoneyRange(nBTC)
                || !MoneyRange(nBitcoin))
                throw std::runtime_error("CWallet::GetCredit() : value out of range");
        }
...
public:
    ...
    mutable bool fAvailableBitcoinCreditCached;
    mutable bool fCreditSplitCached;
    mutable int64_t nDebitCached;
    mutable int64_t nCreditCached;
    mutable int64_t nAvailableCreditCached;
    mutable int64_t nChangeCached;
    mutable int64_t nAvailableBitcoinCreditCached;
    
    mutable int64_t nCredBTCCached;
    mutable int64_t nCredBitcoinCached;
    ...
    void Init(const CWallet* pwalletIn)
    {
        ...
        fAvailableBitcoinCreditCached = false;
        
        nCredBTCCached = 0;
        nCredBitcoinCached = 0;
        ...
        nAvailableBitcoinCreditCached = 0;
        nChangeCached = 0;
        nOrderPos = -1;
    }
    ...
    bool GetCredit(int64_t& nCredBTC, int64_t& nCredBitcoin, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredBTC = nCredBitcoin = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredBTCCached = 0;
            nCredBitcoinCached = 0;
            pwallet->GetCredit(*this, nCredBTCCached, nCredBitcoinCached);
            fCreditSplitCached = true;
        };
        
        nCredBTC = nCredBTCCached;
        nCredBitcoin = nCredBitcoinCached;
        return true;
    }
    ...
    int64_t GetAvailableBitcoinCredit(bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
            return 0;
        
        if (fUseCache && fAvailableBitcoinCreditCached)
            return nAvailableBitcoinCreditCached;
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            if (!IsSpent(i))
            {
                ...
                if (!MoneyRange(nCredit))
                    throw std::runtime_error("CWalletTx::GetAvailableBitcoinCredit() : value out of range");
            };
        };
        
        nAvailableBitcoinCreditCached = nCredit;
        fAvailableBitcoinCreditCached = true;
        return nCredit;
    };

...
#include "bitcoinbridge.moc"

BitcoinBridge::BitcoinBridge(BitcoinGUI *window, QObject *parent) :
    QObject         (parent),
    window          (window),
...
void BitcoinBridge::setClientModel()
{
...
// This is just a hook, we won't really be setting the model...
void BitcoinBridge::setWalletModel()
{
...
// This is just a hook, we won't really be setting the model...
void BitcoinBridge::setMessageModel()
{
    populateMessageTable();
    connectSignals();
}

void BitcoinBridge::copy(QString text)
{
    QApplication::clipboard()->setText(text);
}

void BitcoinBridge::paste()
{
    emitPaste(QApplication::clipboard()->text());
}

// Options
void BitcoinBridge::populateOptions()
{
...
// Transactions
bool BitcoinBridge::addRecipient(QString address, QString label, QString narration, qint64 amount, int txnType, int nRingSize)
{
...
void BitcoinBridge::clearRecipients()
{
    recipients.clear();
}

bool BitcoinBridge::sendCoins(bool fUseCoinControl, QString sChangeAddr)
{
    int inputTypes = -1;
    int nAnonOutputs = 0;
    int ringSizes = -1;
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        switch(rcp.txnTypeInd)
        {
            ...
            case TXT_BTC_TO_ANON:
                formatted.append(tr("<b>%1</b> to SHTT %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BTC, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_ANON:
                formatted.append(tr("<b>%1</b> SHTT, ring size %2 to SHTT %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BTC, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_BTC:
                formatted.append(tr("<b>%1</b> SHTT, ring size %2 to HTT %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BTC, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                break;
            default:
    ...
    switch(sendstatus.status)
    {
        ...
        case WalletModel::SCR_StealthAddressFail:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        case WalletModel::SCR_AmountWithFeeExceedsBitcoinBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                tr("The total exceeds your SHTT balance when the %1 transaction fee is included.").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BTC, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
...
void BitcoinBridge::openCoinControl()
{
...
void BitcoinBridge::updateCoinControlAmount(qint64 amount)
{
    CoinControlDialog::payAmounts.clear();
...
void BitcoinBridge::updateCoinControlLabels(unsigned int &quantity, int64_t &amount, int64_t &fee, int64_t &afterfee, unsigned int &bytes, QString &priority, QString low, int64_t &change)
{
    emitCoinControlUpdate(quantity, amount, fee, afterfee, bytes, priority, low, change);
}

QVariantMap BitcoinBridge::listAnonOutputs()
{
...
void BitcoinBridge::populateTransactionTable()
{
...
void BitcoinBridge::updateTransactions(QModelIndex topLeft, QModelIndex bottomRight)
{
...
void BitcoinBridge::insertTransactions(const QModelIndex & parent, int start, int end)
{
    // New Transactions...
    transactionModel->populateRows(start, end);
}

QString BitcoinBridge::transactionDetails(QString txid)
{
...
// Addresses
void BitcoinBridge::populateAddressTable()
{
...
void BitcoinBridge::updateAddresses(QModelIndex topLeft, QModelIndex bottomRight)
{
    addressModel->poplateRows(topLeft.row(), bottomRight.row());
}

void BitcoinBridge::insertAddresses(const QModelIndex & parent, int start, int end)
{
...
QString BitcoinBridge::newAddress(QString addressLabel, int addressType, QString address, bool send)
{
...
QString BitcoinBridge::lastAddressError()
{
    QString sError;
    ...
    switch(status)
    {
        ...
        case AddressTableModel::INVALID_ADDRESS:
            sError = tr("Invalid Address.");
            break;
        case AddressTableModel::DUPLICATE_ADDRESS:
            sError = tr("Duplicate Address.");
            break;
        case AddressTableModel::WALLET_UNLOCK_FAILURE:
            sError = tr("Unlock Failed.");
            break;
        case AddressTableModel::KEY_GENERATION_FAILURE:
        default:
            sError = tr("Unspecified error.");
            break;
    };
...
QString BitcoinBridge::getAddressLabel(QString address)
{
    return addressModel->atm->labelForAddress(address);
}

void BitcoinBridge::updateAddressLabel(QString address, QString label)
{
...
bool BitcoinBridge::validateAddress(QString address)
{
    return window->walletModel->validateAddress(address);
}

bool BitcoinBridge::deleteAddress(QString address)
{
    return addressModel->atm->removeRow(addressModel->atm->lookupAddress(address));
}

// Messages
void BitcoinBridge::appendMessages(QString messages, bool reset)
{
    emitMessages("[" + messages + "]", reset);
}

void BitcoinBridge::appendMessage(int row)
{
...
void BitcoinBridge::populateMessageTable()
{
...
void BitcoinBridge::insertMessages(const QModelIndex & parent, int start, int end)
{
    while(start <= end)
    {
        appendMessage(start++);
...
bool BitcoinBridge::deleteMessage(QString key)
{
    return window->messageModel->removeRow(thMessage->mtm->lookupMessage(key));
}

bool BitcoinBridge::markMessageAsRead(QString key)
{
    return window->messageModel->markMessageAsRead(key);
}

QString BitcoinBridge::getPubKey(QString address)
{
    return addressModel->atm->pubkeyForAddress(address);;
}

bool BitcoinBridge::setPubKey(QString address, QString pubkey)
{
...
bool BitcoinBridge::sendMessage(const QString &address, const QString &message, const QString &from)
{
    WalletModel::UnlockContext ctx(window->walletModel->requestUnlock());
...
QString BitcoinBridge::createGroupChat(QString label)
{
...
QString BitcoinBridge::joinGroupChat(QString privkey, QString label)
{
    /*
...
QVariantList BitcoinBridge::inviteGroupChat(QString qsaddress, QVariantList invites, QString from)
{
...
void BitcoinBridge::connectSignals()
{
...
QString BitcoinBridge::translateHtmlString(QString string)
{
    int i = 0;
...
QVariantMap BitcoinBridge::userAction(QVariantMap action)
{
...
// Blocks
QVariantMap BitcoinBridge::listLatestBlocks()
{
...
QVariantMap BitcoinBridge::findBlock(QString searchID)
{
...
QVariantMap BitcoinBridge::blockDetails(QString blkHash)
{
...
QVariantMap BitcoinBridge::listTransactionsForBlock(QString blkHash)
{
...
QVariantMap BitcoinBridge::txnDetails(QString blkHash, QString txnHash)
{
...
QVariantMap BitcoinBridge::signMessage(QString address, QString message)
{
    QVariantMap result;
...
QVariantMap BitcoinBridge::verifyMessage(QString address, QString message, QString signature)
{
    QVariantMap result;
...
QVariantMap BitcoinBridge::getNewMnemonic(QString password, QString language)
{
    QVariantMap result;
...
QVariantMap BitcoinBridge::importFromMnemonic(QString inMnemonic, QString inPassword, QString inLabel, bool fBip44, int64_t nCreateTime)
{
...
QVariantMap BitcoinBridge::extKeyAccList() {
    QVariantMap result;
...
QVariantMap BitcoinBridge::extKeyList() {
    QVariantMap result;
...
QVariantMap BitcoinBridge::extKeyImport(QString inKey, QString inLabel, bool fBip44, int64_t nCreateTime)
{
    QVariantMap result;
    ...
    if (eKey58.Set58(sInKey.c_str()) == 0)
    {
        ...
        {
            result.insert("error_msg", "Import failed - Key must begin with HealthierthriceChain prefix.");
            return result;
        }
...
QVariantMap BitcoinBridge::extKeySetDefault(QString extKeyID)
{
    QVariantMap result;
...
QVariantMap BitcoinBridge::extKeySetMaster(QString extKeyID)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
    if (extKeyID.length() == 0)
...
QVariantMap BitcoinBridge::extKeySetActive(QString extKeyID, QString isActive)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();

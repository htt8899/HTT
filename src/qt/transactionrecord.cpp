...
QString TransactionRecord::getTypeLabel(const int &type)
{
    ...
    case RecvWithAddress:
        return BitcoinGUI::tr("Received with");
    case RecvFromOther:
        return BitcoinGUI::tr("Received from");
    case SendToAddress:
    case SendToOther:
        return BitcoinGUI::tr("Sent to");
    case SendToSelf:
        return BitcoinGUI::tr("Payment to yourself");
    case Generated:
        return BitcoinGUI::tr("Mined");
    case RecvBitcoin:
        return BitcoinGUI::tr("Received SHTT");
    case SendBitcoin:
        return BitcoinGUI::tr("Sent SHTT");
    case Other:
        return BitcoinGUI::tr("Other");
    default:
        return "";
    }
}
...
QString TransactionRecord::getTypeShort(const int &type)
{
    ...
    case TransactionRecord::RecvBitcoin:
        return "input";
    case TransactionRecord::SendToAddress:
    case TransactionRecord::SendToOther:
    case TransactionRecord::SendBitcoin:
        return "output";
    default:
        return "inout";
    }
...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet *wallet, const CWalletTx &wtx)
{
    ...
    int64_t nCredBTC, nCredBitcoin;
    wtx.GetCredit(nCredBTC, nCredBitcoin, true);
    int64_t nCredit = nCredBTC + nCredBitcoin;
    ...
    {
        if (nNet > 0 && nCredBitcoin > 0)
        {
            // -- credit
            TransactionRecord sub(hash, nTime, TransactionRecord::RecvBitcoin, "", "", nNet, 0);
        ...
        {
            // -- debit
            TransactionRecord sub(hash, nTime, TransactionRecord::SendBitcoin, "", "", nNet, 0);
        ...
        for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
        {
                ...
                && txout.IsAnonOutput())
            {
                ...
                if (wallet->HaveKey(ckidD))
                {
                    ...
                    sub.type = TransactionRecord::RecvBitcoin;
    ...
    } else
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, wtx.vin)
        {
                ...
                && txin.IsAnonInput())
            {
                ...
                {
                    fAllFromMe = false;
                    break; // display as send/recv SHTT
                };
                continue;
            };
        ...
        if (fAllFromMe)
        {
            ...
            for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
            {
                    ...
                    && txout.IsAnonOutput())
                {
                    ...
                    sub.type = TransactionRecord::SendBitcoin;
                    sub.address = CBitcoinAddress(ckidD).ToString();

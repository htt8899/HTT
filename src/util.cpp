...
#else
    const char* pszModule = "HealthierthriceChain";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\HealthierthriceChain
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\HealthierthriceChain
    // Mac: ~/Library/Application Support/HealthierthriceChain
    // Unix: ~/.HealthierthriceChain
    
    
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "HealthierthriceChain";
#else
    fs::path pathRet;
    ...
    if (pszHome == NULL || strlen(pszHome) == 0)
        ...
        return pathRet / "HealthierthriceChain";
    #else
        // Unix
        return pathRet / ".HealthierthriceChain";
    #endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "HealthierthriceChain.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "HealthierthriceChaind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong HealthierthriceChain will not work properly.");
                    strMiscWarning = strMessage;
                    LogPrintf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("HealthierthriceChain"), CClientUIInterface::BTN_OK | CClientUIInterface::ICON_WARNING);
                }
            }
        }
        if (fDebug) {

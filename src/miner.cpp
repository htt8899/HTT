...
{
    LogPrintf("HealthierthriceChainMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("HealthierthriceChain-miner");
    ...
    try { while (true) {
        ...
        LogPrintf("Running HealthierthriceChainMiner with %llu transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("HealthierthriceChainMiner terminated\n");
        throw;
    }
}

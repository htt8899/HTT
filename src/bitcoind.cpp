...
void ThreadCli()
{
    ...
    printf("HealthierthriceChain CLI ready:\n> ");
    fflush(stdout);
...
void WaitForShutdown(boost::thread_group* threadGroup)
{
    ...
    LogPrintf("HealthierthriceChain shutdown.\n\n");
...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("HealthierthriceChain version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  HealthierthriceChaind [options]                     " + "\n" +
                  "  HealthierthriceChaind [options] <command> [params]  " + _("Send command to -server or HealthierthriceChaind") + "\n" +
                  "  HealthierthriceChaind [options] help                " + _("List commands") + "\n" +
                  "  HealthierthriceChaind [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "HealthierthriceChain:"))
                fCommandLine = true;
...
int main(int argc, char* argv[])
{
    ...
    // Connect HealthierthriceChaind signal handlers
    noui_connect();
